# Execution Prompt — STEP-05 to STEP-14

Drop this prompt in any fresh Claude Code session and Claude will pick up the CV-service rollout exactly where STEP-04 left off, drive each remaining step end-to-end (test-first → implement → polyrepo PR → user merge → monorepo tracker tick), and stop only when STEP-14 ships sc-libs 1.4.0 and the FLOW-07 / UC-02 stamp lands at the top of `progress/README.md`.
...
## Current State (as of start of STEP-05)

| Item | State | Where |
|---|---|---|
| Plan tracker | STEP-00 → STEP-04 ✅; STEP-05 → STEP-14 ⏳ | `progress/README.md` (post-STEP-04 monorepo PR #43) |
...
| Storage adapter | `cv_service.storage.MinioStorageClient` — async, streaming to NamedTemporaryFile, error-mapped (`ObjectNotFoundError`, `StorageTransientError`, `StorageError`); 6 acceptance gates green via Testcontainers MinIO | cv polyrepo `src/cv_service/storage/` |
| Preprocessing pipeline | `cv_service.preprocessing.{loader, pipeline, exceptions, models}` — MIME-sniff loader (page cap), 5-stage OpenCV transform (grayscale → Hough deskew ±15° → NLMD → adaptive threshold → 300-DPI cubic resize), 10 acceptance gates green | cv polyrepo `src/cv_service/preprocessing/` |
| Settings | `minio_*`, `max_pages` (10), `max_image_dimension` (8000), `ocr_langs` (`ukr,en`), `rabbit_prefetch` (8), `rabbitmq_url`, `zipkin_endpoint` | cv polyrepo `src/cv_service/config.py` |
...
| cv polyrepo | `main = STEP-04 merged`, FastAPI + storage + preprocessing, **155.7 MB runtime image**, ml group active, poppler-utils + libmagic1 in builder + runtime | https://github.com/fice-selection-committee/selection-committee-computer-vision |
| CI workflow | `--with ml`, apt installs `poppler-utils libmagic1`, venv cache key `cv-venv-ml-…` | `.github/workflows/cv-service-ci.yml` |
...
|---|---|---|---|
| 05 | yes (cv-service) | yes (progress tracker tick) | — |
...
6. **Update the tracker** — after each step's PRs are merged, edit `progress/README.md` to flip the row from ⏳ TODO to ✅, AND tick "Definition of Done" checkboxes inside the step file. Add a "Regressions Caught" section to the step file documenting any non-obvious fix made during execution. This goes in a small dedicated monorepo PR.
...
12. **Pause and ask before merging anything for the user** — open the PR(s), report the URLs, then wait. The user is the merge gatekeeper UNLESS they have explicitly said "merge everything needed and proceed" / "auto-merge" / equivalent. In that mode, squash-merge each PR after gates are green, sync default branches, and continue. Always report the merged PR URLs in the per-step closing message.

---

## Per-Step Workflow

For each STEP-NN starting with STEP-05:
...
9. **Update the status row** in `progress/README.md` (also kept uncommitted until the polyrepo merge).
...
12. **Report** PR URLs. If auto-merge mode, squash-merge each PR after a green gate; otherwise wait for the user.
13. **After polyrepo merges** — branch the monorepo off freshly-pulled `main`, commit the progress edits as `chore(cv): tick STEP-NN done in tracker`, push, open and (in auto-merge mode) merge the tracker-tick PR.
...
- **STEP-03 lessons**:
  - `MinioStorageClient` accepts URL or bare host:port — wire from `Settings.minio_endpoint` (URL form) without re-parsing.
  - `DownloadedObject.path` is owned by the caller — `try / finally: path.unlink(missing_ok=True)` in the orchestrator (STEP-08).
  - Do **not** import `cv_service.storage` from any module that runs at import time on hosts without Docker.
  - `Settings` already has `minio_access_key` / `minio_secret_key` / `minio_secure`; don't duplicate.
- **STEP-04 lessons**:
  - `pdf2image` shells out to `pdftoppm` / `pdfinfo` at runtime; CI installs `poppler-utils` via apt; Windows local dev uses `winget install oschwartz10612.Poppler` (the binaries land at `%LOCALAPPDATA%\Microsoft\WinGet\Packages\oschwartz10612.Poppler_…\poppler-25.07.0\Library\bin`).
  - PDF tests skip when poppler is missing on PATH (the `require_poppler` fixture in `tests/preprocessing/conftest.py`).
  - `python-magic-bin` is gated by `markers = "sys_platform == 'win32'"` in the ml group; Linux/macOS/Docker resolutions stay clean.
  - Pillow 13 deprecates `Image.fromarray(arr, mode=...)`; with `filterwarnings = ["error"]` the `mode=` kwarg aborts the suite.
  - `Settings.max_pages` (default 10) and `Settings.max_image_dimension` (default 8000) are the single source of truth for the loader and pipeline guards.
  - Dockerfile builder uses `--with ml --without dev`; runtime image is **155.7 MB**. Both stages install `poppler-utils libmagic1`.
  - `cv2.setNumThreads(1)` is set in `tests/preprocessing/conftest.py` for byte-determinism; STEP-08 should pin the same value at process start.
  - Preprocessing pipeline output is `np.ndarray (H, W) uint8`. STEP-05's OCR engine consumes that shape directly — no re-encode needed.
- **`infra/.env.example` carry-over** — add `CV_MINIO_ACCESS_KEY`, `CV_MINIO_SECRET_KEY`, `CV_MINIO_SECURE` in STEP-08 (the first step that actually runs the adapter against the live compose stack).
- **mypy + un-stubbed packages**: `pyproject.toml` already has overrides for `minio.*`, `testcontainers.*`, `urllib3.*`. Add new ones only when a new third-party import lacks stubs — keep the override list minimal.
...
- Approval before merging each step's PR(s) — UNLESS the user has explicitly authorised auto-merge for this run.
...
- Branch base: cv polyrepo `main`.
- Add `paddleocr` (and any companion deps the step file lists) to the **`ml` group**. Pin a known-good version range — verify against the step file.
...
- New module: `src/cv_service/ocr/engine.py` — exposes `recognize(image: np.ndarray) → list[OcrLine]` where `OcrLine = (text: str, bbox: tuple[int,int,int,int], confidence: float)`. Wrap PaddleOCR in a small adapter so the engine type stays decoupled from the SDK.
...
- Image-size impact: this step roughly **doubles** the runtime image (paddleocr + PP-OCRv4 weights). Document the new size in the PR body and `progress/STEP-05-*.md` "Regressions Caught".
- Preprocessing handoff: STEP-04's `PreprocessingPipeline.run()` returns `PreprocessedImage` whose `.image` is `np.ndarray (H, W) uint8`. The OCR engine should accept that shape directly.
...
- Wire download → preprocess → OCR → extract → publish. Use `cv_service.storage.MinioStorageClient` (STEP-03) and `cv_service.preprocessing.PreprocessingPipeline` (STEP-04).
- **Serialize OCR via `asyncio.Lock`** (the lock from STEP-05). Document the throughput trade-off.
- Pin `cv2.setNumThreads(1)` at process start for byte-determinism with the goldens (matches the test-time pin in `tests/preprocessing/conftest.py`).
...
- Resilience: timeouts per stage (download 30s, preprocess 15s, OCR 60s, extract 5s — verify against the step file), classify exceptions into retriable vs terminal, build the `CvDocumentFailedEvent.retriable` flag accordingly. Map storage exceptions: `ObjectNotFoundError` → retriable=False, `StorageTransientError` → retriable=True. Map preprocessing exceptions: `UnsupportedFormatError` → retriable=False, `PreprocessingError` (other) → retriable=False.
...
**Start with STEP-05.** First action: read `progress/STEP-05-ocr-engine.md` end-to-end (`ls progress/STEP-05*` to confirm the filename), then read auto-loaded `server/CLAUDE.md` (it loads the moment you touch anything under `server/`), then write the failing OCR-engine tests in `tests/ocr/test_engine.py` (or wherever the step file specifies) before implementing anything.

Confirm before starting that:
- cv polyrepo `origin/main` HEAD is `feat(cv): step-04 — image preprocessing pipeline …`
- monorepo `origin/main` HEAD is `chore(cv): tick STEP-04 done in tracker (#43)`

If either is behind, STOP and ask the user to merge first — starting STEP-05 from a non-`main` base creates merge churn.

Default to **auto-merge mode** for this run unless the user says otherwise: open each PR, run gates green, squash-merge, sync defaults, continue. Always report each merged PR URL in the per-step closing message.
