# CV-Service Rollout — Execution Prompt (STEP-04 → STEP-14)

This is the self-contained driver prompt for finishing the FICE Computer Vision rollout. Hand it to a fresh Claude Code session and it will execute the remaining steps end-to-end, one per turn, with the user gating PR merges.
...
## Current State (as of start of STEP-04)

| Item | State | Where |
|---|---|---|
| Plan tracker | STEP-00, STEP-01, STEP-02, STEP-03 ✅; STEP-04 → STEP-14 ⏳ | `progress/README.md` (post-STEP-03 monorepo PR) |
...
| Storage adapter | `cv_service.storage.MinioStorageClient` — async, streaming to NamedTemporaryFile, error mapping (`ObjectNotFoundError`, `StorageTransientError`, `StorageError`); 6 acceptance gates green via Testcontainers MinIO | cv polyrepo `src/cv_service/storage/` |
| Settings | adds `minio_access_key`, `minio_secret_key`, `minio_secure` (`minioadmin` / `minioadmin` / `False` local defaults) | cv polyrepo `src/cv_service/config.py` |
...
| cv polyrepo | `main = STEP-03 merged`, FastAPI shell + storage adapter, ~57 MB image | https://github.com/fice-selection-committee/selection-committee-computer-vision |
...
| Env vars | `CV_*` block in `infra/.env.example` (note: missing `CV_MINIO_ACCESS_KEY` / `CV_MINIO_SECRET_KEY` / `CV_MINIO_SECURE` — STEP-08 lands those) | already merged |

**Open carry-over from STEP-03**: `infra/.env.example` lacks the three new MinIO credential / TLS env vars. STEP-08 (orchestrator wiring) is the natural moment to add them; do it there, in the infra polyrepo, alongside any other compose env additions the orchestrator needs.
...
|---|---|---|---|
| 04 | yes (cv-service) | maybe (progress tracker tick) | — |
| 05 | yes (cv-service) | maybe (progress tracker tick) | — |
| 06 | yes (cv-service) | maybe (progress tracker tick) | — |
| 07 | yes (cv-service) | maybe (progress tracker tick) | — |
...
| 12 | yes (cv-service) | maybe (progress tracker tick) | documents-service if publish-side resilience |
...
**Monorepo workflow**: from `D:\develop\fice-selection-committee`, branch off `main`. The progress-tracker tick (`progress/README.md` row + DoD checkboxes inside `progress/STEP-NN-*.md`) is a tiny monorepo PR at the **end** of each step, after the polyrepo PR(s) merge — never bundled inside an unrelated PR.
...
6. **Update the tracker** — after each step's PRs are merged, edit `progress/README.md` to flip the row from ⏳ TODO to ✅, AND tick "Definition of Done" checkboxes inside the step file. Add a "Regressions Caught" section to the step file documenting any non-obvious fix made during execution. This goes in a small dedicated monorepo PR (or batched into the next monorepo PR for that step).
...
12. **Pause and ask before merging anything for the user** — open the PR(s), report the URLs, then wait. The user is the merge gatekeeper.

---

## Per-Step Workflow

For each STEP-NN starting with STEP-04:
...
12. **Report** PR URLs to the user. Wait. Do not start STEP-(NN+1) until all of the current step's PRs are merged on their default branch.
13. **After merge** — branch the monorepo off freshly-pulled `main`, commit the progress edits as `chore(cv): tick STEP-NN done in tracker` (or fold them into the step's monorepo PR if the step has one). Open and report.
...
- **STEP-03 lessons**:
  - `MinioStorageClient` accepts URL or bare host:port — STEP-04+ wire it from `Settings.minio_endpoint` (URL form) without re-parsing.
  - `DownloadedObject.path` is owned by the caller — `try / finally: path.unlink(missing_ok=True)` in the orchestrator (STEP-08).
  - Do **not** import `cv_service.storage` from any module that runs at import time on hosts without Docker — the integration tests skip cleanly when Docker is missing, but production code paths must remain importable everywhere.
...
Next: STEP-(NN+1) <slug> — waiting on your merge of <PR url(s)>
```
...
- Add `opencv-python-headless`, `pdf2image`, `Pillow`, `numpy` to **`ml` poetry group** (large; only needed when ML is active). Pin sensible majors (`opencv-python-headless ^4.10`, `pdf2image ^1.17`, `Pillow ^11.0`, `numpy ^2.1` — verify against the step file).
- Bake `poppler-utils` into the Dockerfile **builder layer** (`pdf2image` invokes `pdftoppm`). Use `apt-get install -y --no-install-recommends poppler-utils && rm -rf /var/lib/apt/lists/*`. Copy the binaries into the runtime layer if the runtime is a clean `python:3.12-slim`.
- New module: `src/cv_service/preprocess/pipeline.py` — orchestrates: format detect (PDF vs image) → PDF→PNG (first page or all pages, per step file) → grayscale → deskew (Hough or moments) → denoise (Gaussian + bilateral) → threshold (Otsu or adaptive). Output: `np.ndarray` shape `(H, W)` dtype `uint8`. Plus a small dataclass for per-stage timing telemetry (used by STEP-09).
- Tests: parametrized fixtures — clean PNG, skewed PNG, multi-page PDF, low-contrast scan. Assert output shape + dtype + non-zero pixel count > threshold. Use `numpy.random.default_rng(seed=…)` for synthetic fixtures so tests are deterministic.
- Add `--with ml` to the dev-time `poetry install` invocation so tests can run locally. CI runs both with-ml and without-ml gates per existing workflow.
- Storage handoff: the orchestrator (STEP-08) reads the file from `DownloadedObject.path`; the preprocess function should accept a `pathlib.Path | bytes | np.ndarray` so STEP-08 doesn't have to re-encode. Pick whatever matches the step file's signature.
...
- Wire download → preprocess → OCR → extract → publish. Use `cv_service.storage.MinioStorageClient` (from STEP-03) to fetch the document.
...
- Resilience: timeouts per stage (download 30s, preprocess 15s, OCR 60s, extract 5s — but verify against the step file), classify exceptions into retriable vs terminal, build the `CvDocumentFailedEvent.retriable` flag accordingly. Map storage exceptions: `ObjectNotFoundError` → retriable=False, `StorageTransientError` → retriable=True.
...
**Start with STEP-04.** First action: read `progress/STEP-04-preprocessing.md` end-to-end (or whatever the actual filename is — `ls progress/STEP-04*`), then read auto-loaded `server/CLAUDE.md` (it loads the moment you touch anything under `server/`), then write the failing preprocessing tests in `tests/preprocess/test_pipeline.py` before implementing anything.

If STEP-03's PR is not yet merged in the cv polyrepo, STOP and ask the user to merge first — STEP-04 imports `cv_service.storage` indirectly (the orchestrator wires it up in STEP-08, but the test fixtures may share infrastructure), and starting STEP-04 from a non-`main` base creates merge churn.
