...
public class SubscribeHandler implements BotCommandHandler {
  ...
  public BotResponse handle(String[] args, Locale locale) {
    ...
    Integer messageThreadId =
        args.length > 2 && !args[2].isEmpty() ? Integer.parseInt(args[2]) : null;

    // Subscribe via DB (primary)
    if (botUserService.isSubscribed(userId)) {
      return BotResponse.text(msg.msg(locale, "notif.subscribe.already"));
    }
    botUserService.toggleSubscription(userId);

    // Also subscribe via legacy file registry for group chat support
    chatRegistry.subscribe(chatId, messageThreadId);

    return BotResponse.text(msg.msg(locale, "notif.subscribe.success"));
  }
}
